...
public class Player extends GameObject  {
    private double xVel = 12, yVel = 12;
    ...
    public void move(ArrayList<Terrain.Tiles> tiles,
                     ArrayList<Terrain.Lava> lava,
                     PopUpFrame pop,
                     KeyHandler key,
                     Audio audio)  {
        if(x <= 0) x = 0;
        if(x >= maxDistX+(double)(playerWidth/2)){
            nextLevel(pop);
        }
        keyType(key, audio);
        tileCollision(tiles);
    ...
    public void nextLevel(PopUpFrame pop) {
        if (GamePanel.level == GamePanel.LEVEL.lv1) {
            GamePanel.level = GamePanel.LEVEL.lv2;
        } else if (GamePanel.level == GamePanel.LEVEL.lv2) {
            GamePanel.level = GamePanel.LEVEL.lv3;
        } else if(GamePanel.level == GamePanel.LEVEL.lv3) {
    ...
    public void lavaCollision(ArrayList<Terrain.Lava> lavas, Audio audio) {
        for (Terrain.Lava lava : lavas) {
            ...
            if (CheckCollision.collisionWithLava(getBoundsTop(), next)) {
                System.out.println(terrain.getPx() + " " + terrain.getPy());
                audio.burningSound();
                x = terrain.getPx();
                y = terrain.getPy();
    ...
    public void drawPlayer(Graphics g) {
        if (flip) g.drawImage(img.getSubimage(24 * forw, 0, 20, 30), (int) x + playerWidth, (int) y, -playerWidth, playerHeight, null);
        else g.drawImage(img.getSubimage(24 * forw, 0, 20, 30), (int) x, (int) y, playerWidth, playerHeight, null);
    }
